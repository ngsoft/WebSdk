...
if (isset($_GET['redis']))
{
     ...
     * @param mixed $command
     *
     * @return false|list<string> false for unbalanced quotes
     */
            ...
            for (; $i < $length; ++$i)
            {
                ...
                if ('"' == $quote)
                {
                    if ('\\' == $c && $i + 1 < $length)
                    {
                        ...
                        if ('x' == $c && $i + 2 < $length && ctype_xdigit(substr($command, $i + 1, 2)))
                        {
    ...
    function escape_value($val)
    {
        ...
        return null !== $val && ( ! is_utf8($val) || preg_match('~^".*"$~s', $val)) ? quote_arg($val) : $val;
    }
    ...
    function unescape_value($val)
    {
        ...
        return $args && 1 == count($args) ? $args[0] : $val;
    }
    ...
    class Db extends SqlDb
    {
        ...
        public function attach($server, $username, $password)
        {
            ...
            if ( ! $this->send('' != $password ? ['AUTH', $username, $password] : ['PING']))
            {
                return $this->error;
            }

            if (DB == '' && preg_match('~redis_version:(.+)~', $this->send(['INFO', 'server']), $match))
            {
        ...
        public function query($query, $unbuffered = false)
        {
            ...
            foreach ((is_array($reply) ? $reply : [$reply]) as $val)
            {
                $rows[] = [$name => escape_value($val)]; // nested arrays are printed as nested tables by select_value()
            }
         ...
         * @param list<list<int|string>> $commands
         *
    ...
    class Driver extends SqlDriver
    {
        ...
        public function select($table, $select, $where, $group, $order = [], $limit = 1, $page = 0, $print = false)
        {
            ...
            {
                $args                = ['SCAN', '' != $next ? $next : 0, 'COUNT', $limit];
                ...
                $_GET['next']        = ('0' != $cursor ? $cursor : ''); // 0 - the iteration has finished
            } else
            {
                $args = $this->send(['KEYS', $where ? $this->whereKeys($where[0])[0] : '*'], $print);
            ...
            if ($args)
            {
                $columns = (in_array('*', $select) ? ['key', 'type', 'value'] : $select);
                $types   = [];
                ...
                if (in_array('value', $columns))
                {
                    ...
                    $query       = $this->query;
                    $values      = $this->send(array_merge(['MGET'], $args), $print && ! $query);
                ...
                {
                    $row      = ['key' => $key, 'type' => $types[$i], 'value' => $values[$i]];
                    $selected = [];
        ...
        public function update($table, $set, $queryWhere, $limit = 0, $separator = "\n")
        {
            ...
            return (bool) queries('MSET ' . implode(' ', $args)); // not the Result, Select would add its num_rows to the affected rows
        }
        ...
        private function send($args, $print)
        {
            ...
            if ($print)
            {
                echo adminer()->selectQuery($this->query, $start, false === $return);
            }
            return $return;
        }
    ...
    {
        return array_map('strval', range(0, connection()->send(['CONFIG', 'GET', 'databases'])[1] - 1));
    }
    ...
    {
        return [['type' => 'PRIMARY', 'columns' => ['key']]];
    }

    function fields($table)
    {
        return [
            'key'   => ['field' => 'key', 'privileges' => ['select' => 1, 'where' => 1, 'insert' => 1]],
            'type'  => ['field' => 'type', 'privileges' => ['select' => 1]], // the type is determined by the command creating the key
            'value' => ['field' => 'value', 'privileges' => ['select' => 1, 'insert' => 1, 'update' => 1]],
        ];
    }
