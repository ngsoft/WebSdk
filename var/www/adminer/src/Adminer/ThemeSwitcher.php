...
class ThemeSwitcher
{
     ...
     * @param $file
     *
     * @return array{theme:string, type:"adminer"|"custom"|"none", select:bool, dark:bool,fix:bool,lang:bool}
     */
    ...
    /**
     * @param string                    $file
     * @param "adminer"|"custom"|"none" $type
     * @param string                    $theme
    ...
    {
        if ('select' !== self::readActionFromBrowser())
        {
            return null;
        }
    ...
    public static function getSelectedTheme()
    {
        if ( ! isset(self::$theme))
        {
            ...
            foreach ($locations as $location)
            {
                ...
                {
                    $isDark   = self::THEME_FILE_DARK == basename($orig);
