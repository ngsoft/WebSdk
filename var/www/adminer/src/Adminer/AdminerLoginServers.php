...
class AdminerLoginServers
{
    ...
    public function __construct($servers = [], $defaultDriver = 'server', $save = false, $dynamic = true, $passwordLess = false)
    {
        ...
        {
            if ( ! empty(env_get($passwordLessKey, '', false)))
            {
                $this->passwordHashes[$passwordLessDriver] = password_hash(env_get($passwordLessKey, '', false), PASSWORD_DEFAULT);
            }
        }
        ...
        if ('mysql' === strtolower($defaultDriver))
        {
        ...
        {
            $this->servers = ['MySql' => ['driver' => $defaultDriver, 'server' => '127.0.0.1']];
        }
        ...
        if (isset($_POST['auth']))
        {
            ...
            if ( ! empty($key))
            {
                ...
                {
                    $this->servers[$key]         = ['driver' => $_POST['auth']['driver'], 'server' => $address];
            ...
            // Sqlite load db file from server name
            if ('sqlite' === $_POST['auth']['driver'] && empty($_POST['auth']['db']) && isset($this->servers[$_POST['auth']['server']]))
            {
                $server = $this->servers[$_POST['auth']['server']]['server'];

                if (is_file($server))
                {
                    $_POST['auth']['db'] = $server;
                }
            }
        }
    }
