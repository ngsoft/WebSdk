...
if (isset($_GET['clickhouse']))
{
    ...
    function clickhouse_type_info($fullType)
    {
        ...
        {
            return [$match[1], isset($match[2]) ? $match[2] : '', $nullable];
        }
    ...
    function clickhouse_field($row)
    {
        ...
        $generated                      = (
            in_array($defaultKind, ['MATERIALIZED', 'ALIAS', 'EPHEMERAL'], true)
            ? $defaultKind
            : ''
        );
    ...
    function clickhouse_field_definition($parts)
    {
        ...
        if (isset($parts[2]) && 'NULL' === trim($parts[2]) && 0 !== strpos($type, 'Nullable('))
        {
            $type = "Nullable({$type})";
        }
        ...
        $comment = (
            isset($parts[5]) && preg_match('~^\s*COMMENT\b~i', $parts[5])
            ? $parts[5]
    ...
    function alter_table($table, $name, $fields, $foreign, $comment, $engine, $collation, $auto_increment, $partitioning)
    {
        if ('' === $table)
        {
            ...
            if ($result && null !== $comment && '' !== $comment)
            {
    ...
    function tables_list()
    {
        ...
        {
            $return[$row['name']] = ('View' === $row['engine'] ? 'VIEW' : 'TABLE');
        }
        return $return;
    }
    ...
    function view($name)
    {
        ...
        if (preg_match('~\s+AS\s+(?=(?:SELECT|WITH)\b)~i', $create, $match, PREG_OFFSET_CAPTURE))
        {
            ...
            return ['select' => substr($create, $offset)];
        }
        return ['select' => ''];
    }
    ...
    {
        return in_array($db, ['system', 'information_schema', 'INFORMATION_SCHEMA'], true);
    }
